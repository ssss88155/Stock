    ...
    try:
        ...
        for m in range(1, target_month + 1):
            ...
            if m >= 2:
                ...
                monthly_report.append({"月份": m_label, "總投入": peak, "差額": diff, "比例": diff_pct, "idx": m})
        ...
        for r in monthly_report:
            ...
            print(f"{YELLOW}{line}{RESET}" if '*' in r["月份"] else line)
        print("-" * 50)
        ...
        for d_str, tasks in history.items():
            dt = parse_date(d_str)
            if dt and this_month_start <= dt <= rep_end:
        ...
        if df_p is not None:
            ...
            print("-" * 110)
            for r in rows:
                line = "".join(pad_to_width(r[k] if isinstance(r[k], str) else f"{r[k]:,.0f}" if '金額' in k or '盈虧' in k else f"{r[k]:.2f}" if '價' in k else f"{r[k]}", w) for k, w in zip(h_cols, h_wids))
