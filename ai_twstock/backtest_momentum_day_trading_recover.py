...
def check_basic_liquidity(sid, date, data):
    stock_info = data.get(sid, {})
    price_data = stock_info.get('price', {})
    sorted_dates = sorted(price_data.keys())
    if date not in sorted_dates: return False
    ...
    if avg_vol_20 < 200000: return False
    curr_p = price_data[date]['close']
    curr_vol = price_data[date].get('Trading_Volume', 0)
    if (curr_p * curr_vol) < 10000000: return False
    return True
...
def decide_buy(momentum_score, dt_signal, risk_ratio, is_winner_buying, pv_alignment, mode='VOLATILITY', sid=None, date=None, data=None):
    if sid and date and data:
        if mode != 'HOLY_GRAIL_BREAKOUT':
            if not check_basic_liquidity(sid, date, data):
                return False, None
        is_aggressive_regime = False
        if '0050' in data:
            ...
            if date in sorted_0050:
                ...
                if idx_0050 >= 20:
                    ...
                    if p0050[date]['close'] > ma20:
                        is_aggressive_regime = True
        else:
            is_aggressive_regime = True
        if not is_aggressive_regime and mode != 'WASH_OUT_DIP':
            return False, None
        stock_info = data.get(sid, {})
        price_data = stock_info.get('price', {})
        sorted_dates = sorted(price_data.keys())
...
def load_all_data(db_path, start_date):
    ...
    cursor.execute("SELECT stock_id, date, open, high, low, close, volume, foreign_buy, sitc_buy FROM daily_prices WHERE date >= ?", (start_date,))
    ...
    cursor.execute("SELECT stock_id, date, trader_name, trader_id, net_qty, is_buy, avg_price FROM broker_details WHERE date >= ?", (start_date,))
    rows = cursor.fetchall()
    for row in rows:
        sid, date = row['stock_id'], row['date']
    ...
    conn.close()
    print(f"[DEBUG] 載入完成，總耗時: {time.time()-t0:.2f}s, 目前 RAM: {get_ram_usage():.1f} MB")
    return data
...
def run_backtest():
    ...
    for idx in range(start_idx, len(all_dates)):
        ...
        if len(portfolio) < top_n:
            ...
            if idx % 20 == 0:
                print(f"[{current_date}] 掃描耗時: {t_scan_end-t_scan_start:.2f}s (產業:{t_sector_end-t_scan_start:.2f}s), RAM: {get_ram_usage():.1f} MB")
