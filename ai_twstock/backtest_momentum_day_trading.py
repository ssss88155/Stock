...
def decide_buy(momentum_score: float, dt_signal: dict, risk_ratio: float, is_winner_buying: bool,
    ...
    if risk_ratio > 0.50: return False, None, f'隔日沖風險過高'
    if pv_alignment and not pv_alignment['is_aligned']: return False, None, f'量價背離'
    eff_threshold = mom_threshold
    if is_winner_buying: eff_threshold *= 0.75
    if pv_alignment and pv_alignment['concentration'] > 0.08: eff_threshold *= 0.85
    if dt_signal['type'] == 'REVERSAL':
        if use_strict_reversal and '買賣指數正向' not in dt_signal['reason']: return False, None, ''
        if dt_signal['strength'] >= 25: return True, 'REVERSAL', dt_signal['reason']
    if dt_signal['type'] == 'FOLLOW' and dt_signal['strength'] >= 20: return True, 'FOLLOW', dt_signal['reason']
    if momentum_score >= eff_threshold: return True, 'MOMENTUM', f'動能分數 {momentum_score:.1f}'
    return False, None, ''
...
def run_momentum_day_trading_backtest(override_config: dict = None, silent: bool = False) -> dict:
    ...
    for idx in range(start_idx, len(all_dates)):
        ...
        for sid, pos in list(portfolio.items()):
            ...
            sell_reason = None
            if gain >= 0.08: sell_reason = f"獲利了結 (+{gain:.1%})"
            elif gain <= -0.05: sell_reason = f"停損 ({gain:.1%})"
